...
分けたくなかったけど、以下のようにテスト用とデプロイ用で分けた。  
dotenvがymlに対応してたら、1つの設定ファイルでまとめられたのに、、、  

* テスト用  
dotenvを用いて、**test/.env** を読み込む。

* デプロイ用  
**env.yml** の値をデプロイ時にLambda関数に設定する。
...
Deploy用のスタックを作成する。このスタックが他のAWSサービスとの依存関係を管理する。
* AWS::IAM::Role  
Deploy用のロールを作成する。
* AWS::S3::Bucket  
Deploy用のバケットを作成する。
